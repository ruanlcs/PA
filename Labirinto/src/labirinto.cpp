...
double Labirinto::calculaCaminho(int& prof, int& NAbert, int& NFech)
{
               ...
               for(dir.col=-1;dir.col<=1;dir.col++)
               {
                   if(dir!=Coord(0,0))
                   {
                       ...
                       if(movimentoValido(atual.getPos(),prox))
                       {
                           suc.setPos(prox);
                           suc.setAntNoh(atual);
                           suc.setG(atual.custoPassado() + norm(dir));
                           suc.heuristica(dest);
                           ...
                           {
                               big= find_if(Aberto.begin(),Aberto.end(),posicao(suc));
   ...
   {
       compr = atual.custoPassado();
       prof = 1;

       while(atual.getAnt()!=orig)
       {
            set(atual.getAnt(),EstadoCel::CAMINHO);
            atual = *find(Fechado.begin(),Fechado.end(),atual.getAnt());
            prof++;
       }
